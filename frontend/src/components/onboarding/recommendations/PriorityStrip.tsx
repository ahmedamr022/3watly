...
const impactBadge: Record<PriorityItem['impact'], string> = {
  High: 'bg-[#E8F8F0] text-[#0E8F55]',
  Medium: 'bg-[#F4EBFB] text-[#7C2BC7]',
  Low: 'bg-[#EEF3FE] text-brand-blue'
};

const tileByKey: Record<string, string> = {
  project: 'bg-[#EEF3FE]',
  certificate: 'bg-[#F4EBFB]',
  course: 'bg-[#E8F8F0]',
  dashboard: 'bg-[#FEF2E4]'
};
...
function PriorityIcon({ itemKey }: {itemKey: PriorityItem['key'];}) {
  if (itemKey === 'project' || itemKey === 'dashboard') {
    return <ChartColumnIcon className="h-[19px] w-[19px] text-brand-blue" strokeWidth={2} aria-hidden="true" />;
  }
  if (itemKey === 'certificate' || itemKey === 'course') {
    return (
      <GraduationCapIcon
        className="h-[19px] w-[19px] text-[#7C2BC7]"
        strokeWidth={1.9}
        aria-hidden="true" />);
...
function tileFor(itemKey: PriorityItem['key']) {
  return tileByKey[itemKey] ?? 'bg-[#E8F8F0]';
}
...
export function PriorityStrip({ items }: PriorityStripProps) {
  return (
    <section
      className={`grid grid-cols-1 divide-y divide-line overflow-hidden lg:grid-cols-4 lg:divide-x lg:divide-y-0 ${surface}`}>
      
      <div className="flex items-start gap-4 p-6">
        <span className="flex h-[54px] w-[54px] shrink-0 items-center justify-center rounded-full bg-[#E8F8F0]">
          <TrophyIcon className="h-[23px] w-[23px] text-brand-green" strokeWidth={1.9} aria-hidden="true" />
        </span>
        <div>
          <h2 className="text-[16px] font-semibold text-[#0E8F55]">Top Priority for You</h2>
          <p className="mt-1.5 text-[13px] font-light leading-[1.55] text-ink-muted">
      ...
      <div key={item.title} className="flex items-start gap-3.5 p-6">
          ...
          <div>
            ...
            <p className="mt-1.5 text-[13px] font-light leading-[1.5] text-ink-muted">
              {item.description}
            </p>
            <span
