...
export function SkillSignalCard({ name, glyph, filled, delay = 0 }: SkillSignalCardProps) {
  ...
  return (
    <motion.div
      ...
      className="flex h-[76px] items-center gap-3.5 rounded-[22px] border border-slate-200/80 dark:border-indigo-500/30 bg-white/95 dark:bg-gradient-to-r dark:from-[#0F172E] dark:via-[#0B1224] dark:to-[#070C1A] backdrop-blur-2xl px-4 shadow-[0_12px_30px_-8px_rgba(0,0,0,0.06)] dark:shadow-[0_0_25px_rgba(99,102,241,0.18),0_15px_35px_-10px_rgba(0,0,0,0.9)] hover:-translate-y-0.5 transition-all duration-300"
    >
      {/* Brand Icon */}
          ...
          {Array.from({ length: 6 }).map((_, index) => (
            <span
              ...
              className={`h-[8px] w-[8px] rounded-full transition-all duration-200 ${
                index < filled 
                  ? 'bg-[#10B981] shadow-sm shadow-emerald-500/50 dark:shadow-[0_0_8px_rgba(16,185,129,0.8)] ring-1 ring-emerald-400/30' 
